CLASS zpru_cl_adf_buffer DEFINITION
  ...
  INTERFACES zpru_if_agent_frw.
    ...
    TYPES: BEGIN OF ts_agent_keys,
             agent_uuid TYPE zpru_agent-agentuuid,
           END OF ts_agent_keys.

    TYPES: BEGIN OF ts_tool_keys,
             agent_uuid TYPE zpru_agent_tool-agentuuid,
             tool_uuid  TYPE zpru_agent_tool-tooluuid,
...
CLASS zpru_cl_adf_buffer IMPLEMENTATION.
  METHOD prep_agent_buffer.
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<ls_key>).
      ...
      IF line_exists( zpru_cl_adf_buffer=>agent_buffer[ instance-agentuuid = <ls_key>-agent_uuid ] ).
        " do nothing
      ELSE.
        SELECT SINGLE @abap_true FROM zpru_agent
          WHERE agentuuid = @<ls_key>-agent_uuid
          INTO @DATA(lv_exists).
  ...
  METHOD prep_tool_buffer.
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<ls_key_child>).
      IF <ls_key_child>-full_key = abap_true.
        IF line_exists( zpru_cl_adf_buffer=>tool_buffer[ instance-agentuuid = <ls_key_child>-agent_uuid
                                                         instance-tooluuid  = <ls_key_child>-tool_uuid ] ).
          " do nothing
        ELSE.
          SELECT SINGLE @abap_true FROM zpru_agent_tool
            WHERE agentuuid = @<ls_key_child>-agent_uuid
              AND tooluuid  = @<ls_key_child>-tool_uuid
            INTO @DATA(lv_exists).
          IF lv_exists = abap_true.
      ...
      ELSE.
        IF     line_exists( zpru_cl_adf_buffer=>agent_buffer[ instance-agentuuid = <ls_key_child>-agent_uuid ] )
           AND VALUE #( zpru_cl_adf_buffer=>agent_buffer[ instance-agentuuid = <ls_key_child>-agent_uuid ]-deleted OPTIONAL ) IS NOT INITIAL.
          " do nothing
        ELSE.
          SELECT SINGLE @abap_true FROM zpru_agent_tool
            WHERE agentuuid = @<ls_key_child>-agent_uuid
          ...
          IF lv_exists_ch = abap_true.
            ...
            IF sy-subrc = 0.
              LOOP AT lt_child_tab ASSIGNING FIELD-SYMBOL(<ls_child>).
                IF NOT line_exists( zpru_cl_adf_buffer=>tool_buffer[ instance-agentuuid = <ls_child>-agentuuid
                                                                     instance-tooluuid  = <ls_child>-tooluuid ] ).
