...
**   <LS_FAILED_TARGET>-fail = <LS_FAILED_agenttype>-%fail.
*      <ls_failed_target>-create = <ls_failed_agenttype>-%create.
*    ENDLOOP.
*
*    LOOP AT ls_cr_REPORTED-zrpruagenttype ASSIGNING FIELD-SYMBOL(<ls_failed_agenttype>).
*      APPEND INITIAL LINE TO cs_failed-agent_type ASSIGNING FIELD-SYMBOL(<ls_failed_target>).
*      <ls_failed_target>-agent_type = <ls_failed_agenttype>-agenttype.
**   <LS_FAILED_TARGET>-fail = <LS_FAILED_agenttype>-%fail.
*      <ls_failed_target>-create = <ls_failed_agenttype>-%create.
...
*     LOOP AT lt_entities ASSIGNING FIELD-SYMBOL(<ls_create>).
*
*    IF    NOT line_exists( gt_agty_buffer[ instance-agent_type = <ls_create>-agent_type ] )
*       OR     line_exists( gt_agty_buffer[ instance-agent_type = <ls_create>-agent_type
*                                           deleted             = abap_true ] ).
*
*      ASSIGN gt_agty_buffer[ instance-agent_type = <ls_create>-agent_type
*                             deleted             = abap_false ] TO FIELD-SYMBOL(<ls_buffer>).
*      IF sy-subrc = 0.
*        DELETE gt_agty_buffer
*               WHERE     instance-agent_type = <ls_buffer>-instance-agent_type
*                     AND deleted             = abap_true.
*      ENDIF.
*
*      APPEND VALUE #(
*          instance-agent_type       = <ls_create>-agent_type
*          instance-short_mem_volume = COND #( WHEN <ls_create>-control-short_mem_volume = abap_true
*                                              THEN <ls_create>-short_mem_volume )
*          instance-discard_strategy = COND #( WHEN <ls_create>-control-discard_strategy = abap_true
*                                              THEN <ls_create>-discard_strategy )
*          instance-summary_strategy = COND #( WHEN <ls_create>-control-summary_strategy = abap_true
*                                              THEN <ls_create>-summary_strategy )
*          instance-max_numb_loop    = COND #( WHEN <ls_create>-control-max_numb_loop = abap_true
*                                              THEN <ls_create>-max_numb_loop )
...
*                                              THEN <ls_create>-local_last_changed )
*          changed                         = abap_true
*          deleted                         = abap_false ) TO gt_agty_buffer ASSIGNING FIELD-SYMBOL(<ls_just_added>).
*
*      fill_agty_admin_fields( EXPORTING iv_during_create = abap_true
*                              CHANGING  cs_agty          = <ls_just_added> ).
*
*      INSERT VALUE #( agent_type = <ls_create>-agent_type ) INTO TABLE cs_mapped-agent_type.
*
...
*                                       iv_v1       = <ls_create>-agent_type )
*                      agent_type = <ls_create>-agent_type ) TO cs_reported-agent_type.
*
*    ELSE.
*      APPEND VALUE #( agent_type = <ls_create>-agent_type
*                      create     = abap_true
