...
CLASS zpru_cl_persistence_sum IMPLEMENTATION.
 ...
 DATA lt_summarization_db TYPE zpru_if_long_mem_persistence=>tt_summarization_db.
    DATA lo_msum_service TYPE REF TO zpru_if_msum_service.
    ...
    TRY.
        lo_msum_service ?= zpru_cl_agent_service_mngr=>get_service( iv_service = `ZPRU_IF_MSUM_SERVICE`
                                                              iv_context = zpru_if_agent_frw=>cs_context-st_persistence_summarize ).
      CATCH zpru_cx_agent_core.
        RAISE SHORTDUMP NEW zpru_cx_agent_core( ).
    ENDTRY.

    lo_msum_service->create_msum( EXPORTING it_msum_create_imp = VALUE #( FOR <ls_in> IN lt_summarization_db
                                                                          ( summaryuuid         = <ls_in>-summaryuuid
                                                                            content             = <ls_in>-content
                                                                            summarycONTENTid          = <ls_in>-summarycONTENTid
                                                                            stage               = <ls_in>-stage
