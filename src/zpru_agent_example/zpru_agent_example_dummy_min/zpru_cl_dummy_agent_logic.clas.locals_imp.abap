...
CLASS lcl_adf_abap_executor IMPLEMENTATION.
  METHOD execute_code_int.
    ...
    SELECT SINGLE * FROM zpru_agent_tool
    WHERE toolname = `NESTED_AGENT` AND
          steptype = `A` AND
          toolprovider = `ZPRU_CL_DUMMY_AGENT_LOGIC`
          INTO @DATA(ls_nested_agent_tool).

    " tool from this agent
    IF ls_nested_agent_tool IS NOT INITIAL.
      APPEND INITIAL LINE TO et_additional_step ASSIGNING <ls_add_step>.
      <ls_add_step>-tooluuid           = ls_nested_agent_tool-tooluuid.
      <ls_add_step>-agentuuid          = ls_nested_agent_tool-agentuuid.
      <ls_add_step>-toolname           = ls_nested_agent_tool-toolname.
      <ls_add_step>-toolprovider       = ls_nested_agent_tool-toolprovider.
      <ls_add_step>-steptype           = ls_nested_agent_tool-steptype.
      <ls_add_step>-toolschemaprovider = ls_nested_agent_tool-toolschemaprovider.
      <ls_add_step>-toolinfoprovider   = ls_nested_agent_tool-toolinfoprovider.
    ENDIF.
...
CLASS lcl_adf_nested_agent IMPLEMENTATION.
  METHOD run_nested_agent_int.
    ...
    IF sy-subrc = 0.
      ...
      lo_util->convert_to_string( EXPORTING ir_abap   = REF #(  <ls_inbounddeliveryheader> )
                                  CHANGING  cr_string = <ls_key_value>-value ).
    ENDIF.
    ...
    IF sy-subrc = 0.
      ...
      lo_util->convert_to_string( EXPORTING ir_abap   = REF #( <ls_outbounddeliveryheader> )
                                  CHANGING  cr_string = <ls_key_value>-value ).
    ENDIF.
    ...
    IF sy-subrc = 0.
      ...
      lo_util->convert_to_string( EXPORTING ir_abap   = REF #(   <lt_inbounddeliveryitems> )
                                  CHANGING  cr_string = <ls_key_value>-value ).
    ENDIF.
    ...
    IF sy-subrc = 0.
      ...
      lo_util->convert_to_string( EXPORTING ir_abap   = REF #( <lt_outbounddeliveryitems> )
                                  CHANGING  cr_string = <ls_key_value>-value ).
    ENDIF.
